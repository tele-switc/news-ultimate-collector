...
def process_source_for_months(conf, months, dedup):
    added_total = 0
    for (y, m) in months:
        # 计算该月区间
        ...
        start_iso = to_iso(month_start)
        end_iso   = to_iso(month_end)

        sitemap = conf.get("sitemap")
        if not sitemap:
            continue
        print(f"[{conf['display_name']}] Backfill {y}-{m:02d} via sitemap: {sitemap}")
        rows = collect_from_sitemap_index(sitemap, start_iso, end_iso, polite_delay=0.5, include_no_lastmod=True) or []
...
def main():
    ONE_SHOT = (os.getenv("ONE_SHOT") or "").strip() == "1"

    # 读取旧状态（分片模式）
    old_state = load_state() if not ONE_SHOT else None

    # 解析时间区间（ONE_SHOT 或输入为空则使用给定/默认；分片则优先沿用旧值）
    ...
    if ONE_SHOT:
        # 一次性全量：所有来源×所有月份（受限于 MAX_URLS_PER_SOURCE_PER_MONTH 与超时）
        total = 0
        for skey, conf in SOURCES.items():
            total += process_source_for_months(conf, months, dedup)
        save_dedup(dedup)
        update_index_indexfile()
        print(f"Backfill one-shot done. New items added: {total}")
        return 0

    # 分片续跑模式
    state = old_state or {"start_iso":start_iso,"end_iso":end_iso,"source_idx":0,"month_idx":0,"complete":False}
    if (start_raw and start_iso != state.get("start_iso")) or (end_raw and end_iso != state.get("end_iso")):
        state = {"start_iso":start_iso,"end_iso":end_iso,"source_idx":0,"month_idx":0,"complete":False}
    ...
    while source_idx < len(sources_keys) and months_processed_this_run < MAX_MONTHS_PER_RUN:
        conf = SOURCES[sources_keys[source_idx]]
        # 本轮要处理的月份切片
        if month_idx >= len(months):
            source_idx += 1
            month_idx = 0
            continue
        slice_months = months[month_idx : min(month_idx+1, len(months))]

        added_total += process_source_for_months(conf, slice_months, dedup)

        month_idx += 1
        months_processed_this_run += 1
        state.update({"start_iso":start_iso,"end_iso":end_iso,"source_idx":source_idx,"month_idx":month_idx,"complete":False})
        save_state(state)
