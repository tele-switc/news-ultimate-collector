...
def import_github_repos(dedup):
    ...
    for cfg in GITHUB_REPOS:
        ...
        try:
            items = collect_repo_items(
                owner=owner, repo=repo,
                branch=cfg.get("branch",""),
                roots=cfg.get("roots") or ["."],
                exts=cfg.get("exts") or [".md",".txt",".html"],
            ...
            for it in items:
                base = make_item(
                    it["url"],
                    it["title"],
                    it.get("source_label") or f"GitHub: {owner}/{repo}",
                    it["published_at"],
                    summary=""
                )
...
def main():
    ...
    # 3) GitHub 仓库导入（许可证白名单才导入全文）
    gh_added = import_github_repos(dedup)
    total_added += gh_added
    print(f"[GitHub] imported items: {gh_added}")
